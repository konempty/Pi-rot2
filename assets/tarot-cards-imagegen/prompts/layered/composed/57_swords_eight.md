...
# arcanaPrompt - 57 Eight of Swords

Source archive: `../ratio-regeneration/by-card/57_swords_eight.md`
Card file: `57_swords_eight.png`
Arcana: `minor`

Use case: stylized-concept
Primary request: Eight of Swords as scene 8 of the Swords suit story: the young Moonwind Academy student stands in a moonlit trial circle, limited by wind ribbons but not truly trapped.
Scene/backdrop: moonlit academy archive room, tall arched windows, blue wind light, star maps on the floor, calm tense atmosphere.
Subject and exact layout: EXACTLY EIGHT SWORDS TOTAL. Make the count obvious with a symmetrical 4+4 layout: FOUR separate upright silver practice swords on the left side of the student and FOUR separate upright silver practice swords on the right side of the student. Keep all eight swords outside the body silhouette, not hidden behind the character, not overlapping the cloak, and fully visible from hilt to tip. Empty space must separate every sword.
Strict negatives: no seven-sword layout, no nine swords, no hidden sword, no partially obscured sword, no extra dagger, no blade-shaped window bar, no sword-like reflection, no sharp decoration that can be counted as a sword.
Character/anatomy: one youthful student in deep violet academy cloak and cream scarf stands at center, natural hands lightly wrapped by blue wind ribbons, no extra fingers. The owl familiar is a small owl bird perched on a book stack or windowsill with round face, beak, feathered wings, and tiny talons only. No human arms, no human hands, no pointing fingers, no sleeves on the owl, no paws.
Pi motif: include two or three readable purple-and-gold Pi Network inspired emblems: one cloak clasp or belt charm, one seal on the trial-circle floor token, and one small seal on a sword guard. The Pi symbol has a top bar, two vertical stems, two small square dots above, and a short attached upturned right hook. No C-shaped loop or circular right-side hole.
Composition/framing: target portrait art-slot aspect ratio close to 817x1014, about 0.806. Keep all eight swords, student, owl bird, and Pi symbols safely away from edges. No tarot frame, no border, no title, no number, no typography, no watermark.
Constraints: inner illustration only; exact sword count; no readable text, no words, no numbers.
Avoid: wrong sword count, owl with human arms, owl with human hands, owl pointing, malformed Pi symbol, C-shaped Pi icon, extra fingers, distorted hands, infographic, diagram, poster layout.
